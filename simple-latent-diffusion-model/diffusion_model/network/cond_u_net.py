...
class ClassConditionalUnetwork(nn.Module):
...
class NumericalConditionalUnetwork(nn.Module):
    def __init__(self, config_path):
        super().__init__()
        with open(config_path, "r") as file:
            config = yaml.safe_load(file)['unet']
        ...
        self.add_module('network', Unet(dim=config['dim'], dim_mults=config['dim_mults'], channels=config['channels']+self.num_conditions, out_dim=config['channels']))
        
    def forward(self, x, t, cond):
        if t.dim() == 0:
            t = x.new_full((x.size(0), ), t, dtype = torch.int, device = x.device)
        n, _, w, h = x.shape
        cond = cond.view(x.size(0), self.num_conditions, 1, 1).expand(n, self.num_conditions, w, h)
        x = torch.cat((x, cond), 1)
        return self.network(x, t)